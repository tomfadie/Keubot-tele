...
# KRITIS UNTUK KEAMANAN: Mengambil Token dari Environment Variable Vercel
# Pastikan ini adalah 'BOT_TOKEN' di Vercel Settings
TOKEN = os.environ.get("BOT_TOKEN") 
if not TOKEN:
    # Di Vercel/GCP, ini akan tercatat sebagai error di Log
...
async def handle_preview_actions(update: Update, context):
    ...
    # --- Tambahkan Logika untuk Ubah Kategori, Nominal, Keterangan di sini jika diperlukan ---
    # Jika tidak ada logika 'ubah', tombol tersebut hanya berfungsi sebagai menu navigasi yang kembali ke state tertentu.
    
    # Untuk sementara, kembali ke menu kategori jika user memilih ubah kategori
        ...
        return GET_NOMINAL # Kembali ke state sebelum GET_NOMINAL, yaitu CHOOSE_CATEGORY
        ...
        return GET_DESCRIPTION # Kembali ke state menunggu nominal
...
# --- FUNGSI ENTRY POINT UNTUK VERCEL (KRITIS) ---

def init_application():
    """Menginisialisasi Application dan Conversation Handler."""
    
    if not TOKEN:
        logging.error("Inisialisasi GAGAL: BOT_TOKEN tidak ditemukan.")
        return None
...
# Variabel Global untuk menyimpan instance Application (Cold Start Optimization)
...
def handler(request):
    ...
    try:
        # Ambil data JSON dari request. Vercel/Flask/GCP menggunakan sintaks ini.
    ...
    try:
        ...
        # Telegram akan mencoba lagi jika kode status bukan 200 OK
        return 'Internal Server Error', 500
